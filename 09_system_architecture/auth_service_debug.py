...
def verify_password(password: str, hashed: str) -> bool:
    ...
    try:
        ...
        print(f"Verify error: {e}", file=sys.stderr, flush=True)
        return False
...
async def login(request: LoginRequest):
    ...
    conn = get_db()
    if not conn:
        print("Database unavailable", file=sys.stderr, flush=True)
    ...
    try:
        ...
        print(f"Found user: {username}, active: {is_active}", file=sys.stderr, flush=True)
        print(f"Password hash: {password_hash[:20]}...", file=sys.stderr, flush=True)
        
        if not is_active:
            raise HTTPException(status_code=403, detail="Account deactivated")
        
        if not verify_password(request.password, password_hash):
            print(f"Password verification failed", file=sys.stderr, flush=True)
            raise HTTPException(status_code=401, detail="Invalid credentials")
